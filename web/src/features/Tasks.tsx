// Zakładka „Zadania": przełącznik trybu (Zadania | Codzienne) — naraz widać tylko jeden composer + jedną listę.
// Tryb „Zadania": composer (treść + termin) + filtry (Wszystkie/Dziś/Nadchodzące) + lista.
// Tryb „Codzienne": composer rutyn + lista rutyn (bez filtrów). Rozdzielenie tłumi mieszanie obu form dodawania.
...
export function TasksPage() {
  ...
  // Domyślnie startujemy na „Zadania"; tryb nie jest zapamiętywany między wejściami (rutyny odhaczasz w „Dziś").
  ...
  return (
    <div className="space-y-4">
      {/* Przełącznik trybu — segmentowany, na pełną szerokość; odróżnia się od pigułek filtrów poniżej.
          Naraz aktywny jest tylko jeden tryb, więc nigdy nie ma dwóch composerów „wpisz + Dodaj" jednocześnie. */}
      <div className="flex gap-1 rounded-[16px] border border-card-border bg-white/[0.04] p-1">
        {MODES.map(({ key, label }) => (
          <button
            key={key}
            type="button"
            onClick={() => setMode(key)}
            className={`flex flex-1 items-center justify-center gap-1.5 rounded-[12px] py-2 text-sm font-bold transition-colors ${
              mode === key ? "accent-gradient text-white" : "text-muted hover:text-ink"
            }`}
          >
            {key === "routines" && <Repeat size={14} strokeWidth={2.5} />}
            {label}
            {key === "routines" && routineCount > 0 && (
              <span className={`text-xs font-semibold ${mode === key ? "text-white/80" : "text-faint"}`}>
                {routineCount}
              </span>
            )}
          </button>
      ...
      {mode === "tasks" ? (
        <>
          ...
          {/* Filtry — należą tylko do trybu „Zadania". */}
        ...
        <>
          {/* Tryb „Codzienne" (rutyny): powtarzalne obowiązki bez godziny i bez push. Odhaczasz je w „Dziś". */}
          ...
          {routineCount === 0 ? (
            <EmptyState
              icon={Repeat}
              title="Brak rutyn"
              description="Dodaj powtarzalne obowiązki bez godziny — pojawią się na górze „Dziś”."
            />
          ) : (
            <ul className="space-y-2.5">
              {(routines ?? []).map((r) => (
